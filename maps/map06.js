...
/** Tổng thời gian nhô đầu → hụp → bay lên (giây) — ngắn để thú trồi khỏi đất nhanh */
const MOLE_RISE_DUR=0.3;
  ...
  for(const h of moleHoles){
    if(!h.animal){
      ...
      if(h.nextSpawn<=0){
        ...
        h.maxAir=Math.max(7.5, 9.5-moleElapsed/24000)+2+Math.random();
      ...
      if(h.fallT>=0){
        ...
        const riseSpeed=1/MOLE_RISE_DUR; // ~0.3s tổng để lên hết
