...
Ngày lập: 2026-07-25  
Cập nhật: 2026-07-25 — xác nhận nhà phát triển: toàn bộ 10 file là giọng tự thu âm.  
...
|---|---:|---|
| 1. Giọng đọc tự thu âm (voice recording gốc) | **10** | Sở hữu của nhà phát triển |
| 2. Âm thanh do AI tạo | **0** | Không dùng AI generate cho các file trong `sounds/` |
| 3. Không rõ nguồn gốc | **0** | — |

### Xác nhận sở hữu
- **Nhà phát triển xác nhận (2026-07-25):** toàn bộ 10 file `.wav` hiện có là giọng đọc do nhà phát triển tự thu âm — sở hữu gốc 100%, không AI generate, không license bên thứ ba.
- Lịch sử git: lần đầu upload `4ecb5cb` (2026-07-04) → chuyển vào `sounds/` ở `633e95c` → bản trim hiện tại `f527171`.
- Không có commit agent/bot tạo hoặc sửa các file `.wav` này.
...
| `godlike.wav` | tự thu âm (voice recording gốc) — sở hữu của nhà phát triển | 2026-07-04 (`4ecb5cb` / `633e95c`) | 2026-07-04 (`f527171`) | PCM mono 44.1 kHz ~0.33s. |
...
`js/audio.js` → `PRAISE_SOUND_FILES` / `speakPraise()`: phát file WAV tương ứng cấp khen combo.
